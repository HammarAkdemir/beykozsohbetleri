...
Zoom: Toplantı 8369840665 için resmi Meeting SDK kullanılır. Sağlanan katılım URL'sindeki pwd değeri gerçek toplantı şifresi değildir. ZOOM_CLIENT_ID, ZOOM_CLIENT_SECRET, ZOOM_MEETING_NUMBER ve gerçek ZOOM_PASSCODE Render ortam değişkenlerinde tanımlanmalıdır; sırları VITE_ değişkenlerine veya kaynak koda koymayın. Yayın ayarlarından yayını açık konuma getirin. SDK izinleri ve toplantı sahibinin hesabı Zoom tarafından doğrulanır; toplantı bağlantısı tek başına yeterli değildir. SDK, izleyiciyi Zoom hesabıyla giriş yaptırmadan role 0 katılımcısı olarak bağlar. Mikrofon, kamera, sohbet ve ekran paylaşımı toplantı sahibi ayarlarından kapatılmalıdır. Kimlikler ve parola arayüzde gösterilmez. Gerçek toplantı bağlantısı gerekli kimlik bilgileri olmadan test edilememiştir.
